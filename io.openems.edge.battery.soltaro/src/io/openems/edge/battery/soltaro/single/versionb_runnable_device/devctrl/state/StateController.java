...
public class StateController {
	...
	public static void init(SoltaroComponent device, Config config) {
		states = new HashMap<>();
		...
		states.put(StateEnum.ERROR, new Error(device, config.maxStartAppempts(), config.startUnsuccessfulDelay(),
				config.errorLevel2Delay()));
	...
	public static State getUndefinedState() {
		return getState(StateEnum.UNDEFINED);
	}
}
