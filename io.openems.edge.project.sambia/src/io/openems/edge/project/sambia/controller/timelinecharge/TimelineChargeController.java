package io.openems.edge.project.sambia.controller.timelinecharge;
...
//	}
	...
	public void run() throws OpenemsNamedException {
		...
		ManagedSymmetricEss ess = this.componentManager.getComponent(this.config.ess_id());
		List<EssDcCharger> chargers = new ArrayList<>();
		for (String chargerId : this.config.charger_ids()) {
			chargers.add(this.componentManager.getComponent(chargerId));
		}
		...
		// Charger Power
		int chargerPower = 0;
		for (EssDcCharger c : chargers) {
			chargerPower += c.getActualPower().value().orElse(0);
		}
		...
		if (floatingChargerPower.avg() >= 1000
			...
			int calculatedPower = ess.getPower().fitValueIntoMinMaxPower(ess, Phase.ALL, Pwr.ACTIVE, 0);
			ess.getSetActivePowerEquals().setNextWriteValue(calculatedPower);
			...
			case CHARGESOC:
				if (essSoc > minSoc) {
					...
					int calculatedPower = ess.getPower().fitValueIntoMinMaxPower(ess, Phase.ALL, Pwr.ACTIVE, -10000);
					ess.getSetActivePowerEquals().setNextWriteValue(calculatedPower);
				}
				break;
			case MINSOC:
